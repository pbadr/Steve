...
public class Voting {
    ...
    public static int setupGameVotes() {
        ...
        for (int i = 0; i < Math.min(3, availableGamesSize); i++) {
            ...
            List<String> availableWorlds = Arrays.asList(pickedGame.getSupportedWorlds());
            ...
            gameWorldVotes.put(pickedGame.getCode(), gameWorldVotesEntry);

            Util.broadcast("Votable game #" + (i + 1) + ": " + GREEN + pickedGame.getName() + RESET +
                    ", maps: " + String.join(", ", gameWorldVotes.get(pickedGame.getCode()).keySet()));
        }
